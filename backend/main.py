...
def load_env_file():
    ...
    for path in paths:
        if os.path.exists(path):
            print(f"[env] Loading environment variables from {path}...")
            try:
                with open(path, "r") as f:
                    for line in f:
                        ...
                        if line and not line.startswith("#") and "=" in line:
                            ...
                            os.environ[k] = v
                print(f"[env] Finished loading variables from {path}")
            except Exception as e:
                print(f"[env] Error reading {path}: {e}")
...
async def post_generate_action_plan(req: ActionPlanRequest):
    ...
    if not lovable_api_key and not openrouter_api_key:
        raise HTTPException(
            status_code=500,
            detail="Neither LOVABLE_API_KEY nor OPENROUTER_API_KEY is configured in the environment."
        )
    ...
    # 1. Try Lovable AI Gateway if LOVABLE_API_KEY is present
    if lovable_api_key:
        print("[AI Action Plan] Attempting generation with Lovable AI Gateway...")
        ...
        try:
            ...
            if res.status_code == 200:
                ...
                print("[AI Action Plan] Successfully generated plan using Lovable AI Gateway")
                return {"plan": choice_text.strip(), "source": "ai"}
            else:
                print(f"[AI Action Plan] Lovable AI Gateway returned error status {res.status_code}: {res.text}")
        except Exception as e:
            print(f"[AI Action Plan] Lovable AI Gateway failed: {e}")
    
    # 2. Try OpenRouter as fallback
    if openrouter_api_key:
        print("[AI Action Plan] Attempting generation with OpenRouter...")
        models_to_try = [
            {"name": "openai/gpt-oss-120b:free", "reasoning": False},
            {"name": "google/gemma-4-26b-a4b-it:free", "reasoning": True},
            {"name": "deepseek/deepseek-r1:free", "reasoning": True},
            {"name": "meta-llama/llama-3.3-70b-instruct:free", "reasoning": False},
            {"name": "google/gemini-2.5-flash:free", "reasoning": False},
            {"name": "qwen/qwen-2.5-72b-instruct:free", "reasoning": False}
        ]
        ...
        for model_info in models_to_try:
            ...
            print(f"[AI Action Plan] Attempting OpenRouter model: {model_name}...")
            try:
                payload = {
                    "model": model_name,
                    "messages": [
                ...
                if res.status_code == 200:
                    data = res.json()
                    choice_text = data["choices"][0]["message"]["content"]
                    print(f"[AI Action Plan] Successfully generated plan using: {model_name}")
                    return {"plan": choice_text.strip(), "source": "ai", "model": model_name}
                else:
                    last_error = f"Model {model_name} returned status {res.status_code}: {res.text}"
                    print(f"[AI Action Plan] Warning: {last_error}")
            except Exception as e:
                last_error = f"Model {model_name} failed with exception: {str(e)}"
                print(f"[AI Action Plan] Error: {last_error}")
                
        return {"plan": "", "source": "error", "error": f"All OpenRouter fallback models failed. Last error: {last_error}", "status": 500}
    
    return {"plan": "", "source": "error", "error": "No working key/endpoint succeeded.", "status": 500}

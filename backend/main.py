...
async def post_generate_action_plan(req: ActionPlanRequest):
    ...
    if not openrouter_api_key:
        raise HTTPException(
            status_code=500,
            detail="No OpenRouter API key set. Please configure OPENROUTER_API_KEY in your environment."
        )
    ...
    for model_info in models_to_try:
        ...
        print(f"[AI Action Plan] Attempting generation with model: {model_name}...")
        try:
            payload = {
                "model": model_name,
                "messages": [
    ...
    return {"plan": "", "source": "error", "error": f"All fallback models failed. Last error: {last_error}", "status": 500}

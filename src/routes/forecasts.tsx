  ...
  async function generate() {
    ...
    try {
      ...
      if (res.source === "ai" && res.plan) {
        setAiPlan(res.plan);
        if (res.model) {
          setAiModel(res.model.split("/").pop() || res.model);
        }
      } else {
        setErr(
          res.error || (res.status === 429
            ? "AI rate limit reached — try again shortly."
            : "AI unavailable — showing the baseline plan.")
        );
      }
    } catch (e: any) {
      setErr(e.message || "AI unavailable — showing the baseline plan.");
    } finally {
      setLoading(false);
    }
  }
    ...
    <Card title="AI Action Plan" icon={<Megaphone className="size-4" />} className={className}>
      <div className="mb-3 flex items-center gap-2 shrink-0">
        ...
        <span className={`rounded-md border px-2 py-1 text-[10px] font-semibold uppercase tracking-wide ${aiPlan ? "border-accent/40 bg-accent/10 text-accent" : "border-border bg-input/30 text-muted-foreground"}`}>
          {aiPlan ? (aiModel ? `AI · ${aiModel}` : "AI-generated") : "Baseline"}
        </span>
      </div>
      {err && (

...
import { EVENT_TYPES, predict, fmtHour, type EventType, type Venue } from "@/lib/ai/gridmind";
import { DEFAULT_PLACE } from "@/lib/locations";
import { LocationSearch } from "@/components/LocationSearch";
import { useEvents, uid, type PlannedEvent } from "@/store/store";
  ...
  head: () => ({
    meta: [
      { title: "Event Planner — GridMind AI" },
      {
        name: "description",
        content:
          "Schedule major events, track operational status, and capture post-event feedback to improve future GridMind AI predictions.",
      },
    ],
  }),
...
function NewEventForm({ onAdd }: { onAdd: (e: PlannedEvent) => void }) {
  ...
  return (
    <div className="h-fit rounded-2xl border border-border panel-glass p-5">
      ...
      <button
        onClick={submit}
        className="mt-2 flex w-full items-center justify-center gap-2 rounded-xl bg-primary px-4 py-3 text-sm font-bold text-primary-foreground shadow-glow transition hover:brightness-110"
      >
  ...
  return (
    <div className="rounded-2xl border border-border panel-glass p-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span
              className={`rounded-md border px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide ${statusStyle[e.status]}`}
            >
              {e.status}
            </span>
          </div>
          ...
          <div className="mt-1 flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground">
            <span className="flex items-center gap-1">
              <MapPin className="size-3" /> {e.date}
            </span>
            <span className="flex items-center gap-1">
              <Clock className="size-3" /> {fmtHour(e.hour)} · {e.durationHr}h
            </span>
            <span className="flex items-center gap-1">
              <Users className="size-3" /> {e.attendees.toLocaleString()}
            </span>
            <span className="flex items-center gap-1">
              <Activity className="size-3" /> Predicted {p.delayMin} min · {p.severity}
            </span>
          </div>
        </div>
        <button
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        ...
        <Action
          active={feedbackOpen || e.status === "Completed"}
          onClick={onFeedback}
      ...
      {e.outcome && !feedbackOpen && (
        <div className="mt-3 rounded-lg border border-success/30 bg-success/5 p-3 text-xs">
          <div className="flex flex-wrap items-center gap-3 text-muted-foreground">
            <span>
              Actual delay <strong className="text-foreground">{e.actualDelayMin} min</strong>
            </span>
            <span className="flex items-center gap-1">
              Predicted {p.delayMin} <ArrowRight className="size-3" /> Actual {e.actualDelayMin}
            </span>
            <span>
              Outcome <strong className="text-foreground">{e.outcome}</strong>
            </span>
          </div>
          {e.lesson && (
            <p className="mt-2 flex items-start gap-1.5 text-muted-foreground">
              <Brain className="mt-0.5 size-3.5 text-primary" /> {e.lesson}
            </p>
          )}
        </div>
      )}
...
function FeedbackForm({
  ...
  e: PlannedEvent;
  predicted: number;
  onSave: (patch: Partial<PlannedEvent>) => void;
}) {
  ...
  const [outcome, setOutcome] = useState<NonNullable<PlannedEvent["outcome"]>>(
    ...
    <div className="mt-3 rounded-xl border border-primary/30 bg-primary/5 p-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <FieldLabel label={`Officers used · ${officers}`}>
          <input
            ...
            max={60}
            value={officers}
            onChange={(ev) => setOfficers(+ev.target.value)}
            className="w-full accent-[var(--primary)]"
          />
        </FieldLabel>
      </div>
        ...
        <div className="flex gap-2">
          {(["Successful", "Partial", "Strained"] as const).map((o) => (
            <button
              ...
              className={`flex-1 rounded-lg border px-2 py-2 text-xs font-semibold transition ${outcome === o ? "border-primary bg-primary/15 text-primary" : "border-border bg-input/40 text-muted-foreground"}`}
            >
              {o}
            </button>
          ))}
      ...
      <FieldLabel label="Lesson learned">
        <textarea
          ...
          rows={2}
          placeholder="e.g. Pre-position 2 tow trucks near Gate 3 next time."
          className={inputCls}
        />
      </FieldLabel>
      <button

...
const SCENARIO_ACTIONS: ScenarioAction[] = [
  {
    key: "close-road",
    label: "Close Road",
    description: "Reroute traffic from one corridor.",
    icon: <Cone className="size-4" />,
  },
  {
    key: "deploy-officers",
    label: "Deploy Officers",
    description: "Add 20 officers to improve intersection handling.",
    icon: <ShieldAlert className="size-4" />,
  },
  {
    key: "diversion-b",
    label: "Diversion Route B",
    description: "Open an alternate bypass route.",
    icon: <ArrowRight className="size-4" />,
  },
  {
    key: "signal-timing",
    label: "Signal Timing",
    description: "Optimize green time at critical lights.",
  ...
  switch (scenario) {
    case "close-road":
      return {
        ...
        summary:
          "Road closure shifts traffic off the primary link and reduces average delay for the simulated event.",
        stats: [
    ...
    case "deploy-officers":
      return {
        ...
        recommendation: "Recommended",
        summary:
          "Additional officers reduce queue length and lower delay with better traffic flow control.",
        stats: [
    ...
    case "diversion-b":
      return {
        ...
        recommendation: "Recommended",
        summary:
          "Route B shifts traffic away from the event footprint, saving fuel and reducing delay.",
        stats: [
          { label: "Traffic shifted", value: "8,000 vehicles" },
    ...
    case "signal-timing":
      return {
        ...
        recommendation: "Recommended",
        summary:
          "Signal timing adjustments improve clearance and lower average delay along the corridor.",
        stats: [
          { label: "Signal clearance", value: "-22%" },
    ...
    default:
      return {
        ...
        summary:
          "Use the buttons to simulate closure, officer deployment, diversion routing, or signal adjustment.",
        stats: [
  ...
  head: () => ({
    meta: [
      { title: "What-If Simulator — GridMind AI" },
      {
        name: "description",
        content:
          "Drag attendance, timing and event type to instantly see how predicted delay, congestion and required police resources change.",
      },
    ],
  }),
        ...
        <div className="mb-6 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Scrub the inputs and watch delay, congestion and required resources react in real
              time.
            </p>
          </div>
        </div>
          ...
          <div className="h-fit rounded-2xl border border-border panel-glass p-5">
            ...
            <div className="mt-2 flex gap-2">
              <button
                ...
                className={`flex-1 rounded-lg border px-3 py-2 text-xs font-semibold transition ${planned ? "border-success/40 bg-success/10 text-success" : "border-border bg-input/40 text-muted-foreground"}`}
              >
                Planned
              </button>
              <button
                onClick={() => setPlanned(false)}
                className={`flex-1 rounded-lg border px-3 py-2 text-xs font-semibold transition ${!planned ? "border-warning/40 bg-warning/10 text-warning" : "border-border bg-input/40 text-muted-foreground"}`}
              >
                Unplanned
              </button>
            </div>
            ...
            <p className="mt-4 text-[11px] text-muted-foreground">
              Deltas compare against a baseline of{" "}
              <span className="text-foreground font-semibold">20,000</span> attendees with the same
              venue and timing.
            </p>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="rounded-2xl border border-border panel-glass p-5">
              <div className="mb-4 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    Simulate interventions and see recommendation-driven outcomes.
                  </p>
                </div>
                <span className="rounded-full border border-border bg-input/50 px-3 py-1 text-[11px] uppercase text-muted-foreground">
                  {scenarioOutcome.title}
                </span>
              </div>
            ...
            <div className="rounded-2xl border border-border panel-glass p-5">
              <h2 className="mb-4 text-sm font-bold uppercase tracking-wide text-muted-foreground">
                Resource impact vs baseline
              </h2>
              <div className="grid gap-3 sm:grid-cols-3">
                <ResourceDelta
                  label="Officers"
                  icon={<ShieldAlert className="size-4" />}
                  base={base.resources.officers}
                  now={sim.resources.officers}
                />
                <ResourceDelta
                  label="Barricades"
                  icon={<Cone className="size-4" />}
                  base={base.resources.barricades}
                  now={sim.resources.barricades}
                />
                <ResourceDelta
                  label="Tow trucks"
                  icon={<ArrowRight className="size-4" />}
                  base={base.resources.towTrucks}
                  now={sim.resources.towTrucks}
                />
              </div>
              {sim.resources.deficit > 0 && (
                <div className="mt-4 flex items-center gap-2 rounded-xl border border-critical/40 bg-critical/10 px-4 py-3 text-sm text-critical">
                  ...
                  <span>
                    <strong>{sim.resources.deficit} officers short</strong> at this attendance —
                    request city reserve before the event.
                  </span>
                </div>
              )}
            </div>
            ...
            <div className="rounded-2xl border border-border panel-glass p-5">
              <div className="mb-3 flex items-center justify-between">
                ...
                <span
                  ...
                  style={{ color: sevColor(sim.score) }}
                >
                  {sim.severity}
                </span>
              </div>
              <div className="h-3 w-full overflow-hidden rounded-full bg-input/60">
                <div
                  ...
                  style={{ width: `${sim.score}%`, background: sevColor(sim.score) }}
                />
              </div>
              <p className="mt-3 text-xs text-muted-foreground">
                Recovery time at this scenario:{" "}
                <span className="text-foreground font-semibold">{sim.recoveryHr} h</span>. Closest
                prior event:{" "}
                <span className="text-foreground font-semibold">{sim.similar.event.id}</span> (
                {(sim.similar.match * 100).toFixed(0)}% match).
              </p>
            </div>
          </div>
        </div>
...
const inputCls =
  "w-full rounded-lg border border-border bg-input/60 px-3 py-2.5 text-sm outline-none focus:border-primary";
...
function DeltaCard({
  ...
  return (
    <div className="rounded-2xl border border-border panel-glass p-4">
      ...
      <p className="text-mono text-3xl font-bold">
...
function Trend({ diff, unit }: { diff: number; unit: string }) {
  ...
  return (
    <p
      className={`mt-0.5 flex items-center gap-1 text-[11px] font-semibold ${up ? "text-critical" : "text-success"}`}
    >
      {up ? <TrendingUp className="size-3" /> : <TrendingDown className="size-3" />}
      {up ? "+" : ""}
      {diff}
      {unit && ` ${unit}`} vs baseline
    </p>
  );
}
import { createFileRoute } from "@tanstack/react-router";

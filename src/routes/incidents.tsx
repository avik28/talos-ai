...
import { computeEscalation, ESCALATION_STYLE } from "@/lib/simulation/escalation";
import { LocationSearch } from "@/components/LocationSearch";
import { DEFAULT_PLACE } from "@/lib/locations";
import type { Venue } from "@/lib/ai/gridmind";
  ...
  head: () => ({
    meta: [
      { title: "Incident Reporting — GridMind AI" },
      {
        name: "description",
        content:
          "Report and track live traffic incidents — accidents, breakdowns, signal failures and crowd surges — and dispatch response across Bengaluru.",
      },
    ],
  }),
    ...
    <div className="min-h-screen grid-bg">
      ...
      <main className="mx-auto max-w-7xl px-4 pb-24 pt-6 md:px-6">
        <div className="mb-6 flex items-center gap-3">
          <div className="flex size-11 items-center justify-center rounded-xl bg-critical/15 text-critical">
            <Siren className="size-5" />
          </div>
          <div>
            <h1 className="text-xl font-extrabold tracking-tight">Incident Reporting</h1>
            <p className="text-xs text-muted-foreground">
              Log live field incidents — the Smart Escalation Engine auto-prioritises them by
              severity, age and clustering.
            </p>
          </div>
        </div>
          ...
          <div className="mb-6 rounded-2xl border border-critical/40 bg-critical/10 p-5">
            <div className="flex items-center gap-2 text-critical">
              ...
              <h2 className="text-sm font-bold uppercase tracking-wide">
                Auto-escalated — {escalated.length} critical alert(s)
              </h2>
            </div>
            <div className="mt-3 space-y-2">
              {escalated.slice(0, 3).map(({ i, esc }) => (
                <div
                  key={i.id}
                  className="rounded-xl border border-critical/30 bg-card/40 p-3 text-xs"
                >
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <span
                      className={`rounded-md border px-2 py-0.5 text-[10px] font-bold uppercase ${ESCALATION_STYLE[esc.level]}`}
                    >
                      {esc.level}
                    </span>
                  </div>
              ...
              return (
                <div key={i.id} className="rounded-2xl border border-border panel-glass p-4">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <span
                          className={`rounded-md border px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide ${sevStyle[i.severity]}`}
                        >
                          {i.severity}
                        </span>
                        <span
                          className={`rounded-md border px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide ${statusStyle[i.status]}`}
                        >
                          {i.status}
                        </span>
                        {i.status !== "Resolved" && (
                          <span
                            className={`flex items-center gap-1 rounded-md border px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide ${ESCALATION_STYLE[esc.level]}`}
                          >
                      ...
                      <div className="mt-1 flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground">
                        ...
                        <span className="flex items-center gap-1">
                          <Clock className="size-3" /> {timeAgo(i.createdAt)}
                        </span>
                        <span className="flex items-center gap-1">
                          <Radio className="size-3" /> {i.reporter}
                        </span>
                      </div>
                      {i.description && (
                        <p className="mt-2 text-xs text-foreground/90">{i.description}</p>
                      )}
                    </div>
                    <button
                      onClick={() => removeIncident(i.id)}
                      className="rounded-lg border border-border p-2 text-muted-foreground transition hover:border-critical/40 hover:text-critical"
                    >
                      <Trash2 className="size-4" />
                    </button>
                  </div>
                  {i.status !== "Resolved" &&
                    (esc.level === "High Priority" || esc.level === "Critical") && (
                      <div className="mt-3 rounded-xl border border-warning/30 bg-warning/5 p-3 text-xs">
                        <div className="mb-1 flex items-center gap-1.5 font-bold text-warning">
                          <TrendingUp className="size-3.5" /> Escalation rationale
                        </div>
                        <p className="text-muted-foreground">{esc.reasons.join(" · ")}</p>
                        <div className="mt-2 flex flex-wrap gap-2 text-foreground">
                          <span className="flex items-center gap-1 rounded-md bg-input/40 px-2 py-1">
                            <ShieldAlert className="size-3" />+{esc.recommend.officers} officers
                          </span>
                          {esc.recommend.towTrucks > 0 && (
                            <span className="flex items-center gap-1 rounded-md bg-input/40 px-2 py-1">
                              <Truck className="size-3" />+{esc.recommend.towTrucks} tow
                            </span>
                          )}
                          {esc.recommend.ambulances > 0 && (
                            <span className="flex items-center gap-1 rounded-md bg-input/40 px-2 py-1">
                              <Ambulance className="size-3" />+{esc.recommend.ambulances} ambulance
                            </span>
                          )}
                        </div>
                      </div>
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    <Action
                      active={i.status === "Dispatched"}
                      onClick={() => updateIncident(i.id, { status: "Dispatched" })}
...
function ReportForm({ onAdd }: { onAdd: (i: Incident) => void }) {
  ...
  return (
    <div className="h-fit rounded-2xl border border-border panel-glass p-5">
        ...
        <div className="grid grid-cols-4 gap-2">
          {SEVERITIES.map((s) => (
            <button
              ...
              className={`rounded-lg border px-1 py-2 text-[11px] font-semibold transition ${severity === s ? sevStyle[s] : "border-border bg-input/40 text-muted-foreground"}`}
            >
              {s}
            </button>
          ))}
      ...
      <button
        onClick={submit}
        className="mt-1 flex w-full items-center justify-center gap-2 rounded-xl bg-critical px-4 py-3 text-sm font-bold text-primary-foreground shadow-glow transition hover:brightness-110 disabled:opacity-50"
      >
        {done ? (
          <>
            <CheckCircle2 className="size-4" /> Reported
          </>
        ) : (
          <>
            <Siren className="size-4" /> File report
          </>
        )}
      </button>
    </div>
...
function Action({
  ...
  return (
    <button
      onClick={onClick}
      className={`flex items-center gap-1.5 rounded-lg border px-3 py-1.5 text-xs font-semibold transition ${active ? "border-primary bg-primary/15 text-primary" : "border-border bg-input/30 text-muted-foreground hover:text-foreground"}`}
    >
      {icon}
      {children}
    </button>

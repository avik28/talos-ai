...
import {
  ...
  type Venue,
  VENUES,
} from "@/lib/ai/gridmind";
import { ALL_PLACES, DEFAULT_PLACE } from "@/lib/locations";
import { generateActionPlan } from "@/lib/ai/actionplan.functions";
import { fetchHistoricalData, type HistoricalData } from "@/lib/simulation/historical.functions";
import { useEvents, useIncidents, uid, type Incident } from "@/store/store";
...
import {
  ...
  type Route,
  type WhatIfResponse,
} from "@/lib/simulation/diversionEngine";
...
export function UnifiedCommandCenter({ defaultTab }: UnifiedCommandCenterProps) {
  ...
  function getCorridorFromIncident(incident: Incident): string {
    ...
    if (
      loc.includes("mg road") ||
      loc.includes("cubbon") ||
      loc.includes("richmond") ||
      loc.includes("brigade") ||
      loc.includes("queens")
    ) {
      return "mg_road";
    }
    if (
      loc.includes("hebbal") ||
      loc.includes("rt nagar") ||
      loc.includes("palace") ||
      loc.includes("sadashivanagar") ||
      loc.includes("sanjaynagar")
    ) {
      return "hebbal";
    }
    if (
      loc.includes("orr") ||
      loc.includes("outer ring") ||
      loc.includes("marathahalli") ||
      loc.includes("kr puram") ||
      loc.includes("whitefield") ||
      loc.includes("hoodi") ||
      loc.includes("manyata") ||
      loc.includes("sarjapur")
    ) {
      return "orr";
    }
    return "mg_road";
  ...
  function generateDynamicCorridor(incident: Incident): Corridor {
    const place =
      ALL_PLACES.find((p) => incident.location.toLowerCase().includes(p.name.toLowerCase())) ??
      DEFAULT_PLACE;
    ...
    const startPt: [number, number] = [
      incLat - rLat * Math.cos(phi),
      incLng - rLng * Math.sin(phi),
    ];
    ...
    const wp2: [number, number] = [
      incLat + 0.018 * Math.cos(perpPhi),
      incLng + 0.018 * Math.sin(perpPhi),
    ];
    ...
    return {
      ...
      baselineClearanceMin:
        incident.severity === "Critical" ? 60 : incident.severity === "High" ? 45 : 30,
      stacks: {
  ...
  useEffect(() => {
    ...
    if (selectedIncident.kind === "VIP Movement" || selectedIncident.kind === "Crowd Surge") {
      ...
      selectedIncident.kind === "Waterlogging" ||
      selectedIncident.kind === "Signal Failure" ||
      isHighSev
    ) {
      stack = "beta";
    }
  ...
  const predictionInput: PredictionInput | null = selectedIncident
    ? {
      type:
        selectedIncident.kind === "VIP Movement"
          ? "VIP Movement"
          : selectedIncident.kind === "Crowd Surge"
            ? "Protest"
            : "Roadwork / Diversion",
      venueId:
        VENUES.find((v) => selectedIncident.location.toLowerCase().includes(v.name.toLowerCase()))
          ?.id ?? DEFAULT_PLACE.id,
      location:
        VENUES.find((v) =>
          selectedIncident.location.toLowerCase().includes(v.name.toLowerCase()),
        ) ?? DEFAULT_PLACE,
      attendees:
    ...
    setTimeout(() => {
      setPrediction(
        predict(
          predictionInput,
          history
            ? { incidents: history.incidents, stations: history.stations, plans: history.plans }
            : undefined,
        ),
      );
      setAnalyzing(false);
  ...
  const [commandInput, setCommandInput] = useState<string>(
    selectedIncident ? `Deploy officers for ${selectedIncident.kind}` : "",
  );
  const [consoleMessages, setConsoleMessages] = useState<
    Array<{ sender: "user" | "system"; text: string }>
  >([
    {
      sender: "system",
      text: "GridMind Diversion CLI v1.0. Ready. Type queries to override simulation (e.g. 'Deploy 12 officers').",
    },
  ]);
  ...
  const resources = useMemo(() => {
    ...
    if (!activeStack || activeStack.length === 0)
      return { barricades: 0, officers: 0, towTrucks: 0 };
      ...
      const fetchRoute = async (routeId: string, pts: [number, number][]) => {
        ...
        try {
          ...
          if (selectedIncident) {
            const place =
              ALL_PLACES.find((p) =>
                selectedIncident.location.toLowerCase().includes(p.name.toLowerCase()),
              ) ?? DEFAULT_PLACE;
  ...
  function drawMapElements() {
    ...
    liveIncidents.forEach((inc) => {
      ...
      if (place) {
        ...
        L.marker([place.lat, place.lng], { icon: customIcon })
          .bindTooltip(
            `<b>${isSelected ? "ACTIVE CONTEXT: " : ""}Incident: ${inc.kind}</b><br/>Severity: ${inc.severity}<br/>Location: ${inc.location}<br/>Description: ${inc.description}`,
            { direction: "top" },
          )
          .addTo(group);
      }
    ...
    if (selectedCorridor) {
      ...
      L.marker([selectedCorridor.lat, selectedCorridor.lng], { icon })
        .bindTooltip(
          `<b>${selectedCorridor.name}</b><br/>Base Load: ${(selectedCorridor.baseLoad * 100).toFixed(0)}%`,
          { direction: "top" },
        )
        .addTo(group);
      ...
      activeStack.forEach((route, index) => {
        ...
        L.polyline(pointsToDraw, {
          ...
          .bindTooltip(
            `<b>${route.name}</b><br/>Status: ${route.status}<br/>Distance: ${route.distanceKm} km`,
            { direction: "top" },
          )
          .addTo(group);
      });
      ...
      effectiveClosedRoads.forEach((road) => {
        ...
        if (road.toLowerCase().includes("mg road")) {
          ...
          const place = ALL_PLACES.find(
            (p) =>
              p.name.toLowerCase() === road.toLowerCase() ||
              p.name.toLowerCase().includes(road.toLowerCase()),
          );
          if (place) {
            closedCoords = [
              [place.lat - 0.0006, place.lng - 0.0006],
              [place.lat + 0.0006, place.lng + 0.0006],
            ];
        ...
        if (closedCoords.length > 0) {
          L.polyline(closedCoords, {
            ...
            .bindTooltip(`<b style="color:var(--critical);">${label}</b>`, {
              permanent: true,
              direction: "top",
            })
            .addTo(group);
    ...
    (async () => {
      ...
      try {
        ...
        if (!globalMap) {
          ...
          globalL = L;
          const map = L.map(cachedMapDiv, { zoomControl: true, attributionControl: false }).setView(
            [12.9716, 77.5946],
            12,
          );
        ...
        setTimeout(() => {
          if (!cancelled && globalMap) globalMap.invalidateSize();
        }, 50);
        setTimeout(() => {
          if (!cancelled && globalMap) globalMap.invalidateSize();
        }, 300);
  ...
  async function runQuery(cmd: string) {
    ...
    try {
      ...
      setConsoleMessages((prev) => [
        ...
        {
          sender: "system",
          text: `Scenario Projection Generated: ${data.title}. Congestion: ${data.congestionBefore}% → ${data.congestionAfter}%. ${data.description}`,
        },
      ]);
    } catch (error) {
      ...
      if (
        cmd.toLowerCase().includes("close mg road") ||
        cmd.toLowerCase().includes("mg road closed")
      ) {
        if (!closedRoads.includes("MG Road")) setClosedRoads((prev) => [...prev, "MG Road"]);
      }
      if (
        cmd.toLowerCase().includes("close gate 3") ||
        cmd.toLowerCase().includes("gate 3 exit closed")
      ) {
        if (!closedRoads.includes("Gate 3 Exit"))
          setClosedRoads((prev) => [...prev, "Gate 3 Exit"]);
      }
      if (
        cmd.toLowerCase().includes("convert queens road") ||
        cmd.toLowerCase().includes("queens road one-way")
      ) {
        if (!closedRoads.includes("Queens Road"))
          setClosedRoads((prev) => [...prev, "Queens Road"]);
      }
      const officerMatch =
        cmd.match(/(?:deploy|place|add|use)\s+(\d+)\s+officer/i) || cmd.match(/(\d+)\s+officer/i);
      ...
      setConsoleMessages((prev) => [
        ...
        {
          sender: "system",
          text: `[FALLBACK] Scenario Projection Generated: ${res.title}. Congestion: ${res.congestionBefore}% → ${res.congestionAfter}%. ${res.description}`,
        },
      ]);
    }
    setCommandInput("");
      ...
      return prevCorridors.map((c) => {
        if (c.id === selectedCorridorId) {
          ...
          if (newStrikes >= 3) {
            ...
            toast.error(
              `3-Strike Threshold breached on ${primaryRoute.name}! Demoting to Penalty Box. Promoted ${activeRoutes[1].name} to Primary.`,
            );
            return {
              ...c,
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Tactical Sandbox driven by live incident dispatching, with integrated AI operational
                assessment.
              </p>
            </div>
          </div>
          ...
          <div className="mb-6 rounded-2xl border border-primary/40 bg-primary/5 p-5">
            <div className="flex items-center gap-2 text-primary">
              ...
              <h2 className="text-sm font-bold uppercase tracking-wide">
                Phase 0: Bootstrapping Stacks...
              </h2>
            </div>
            <p className="mt-1 text-xs text-muted-foreground">
              Pre-aging routing stacks using 8,100+ historical incident records.
            </p>
        ...
        <section className="grid gap-6 lg:grid-cols-[380px_1fr]">
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center justify-between">
                          ...
                          <span
                            className={`rounded-md border px-1.5 py-0.5 text-[8px] font-bold uppercase ${inc.severity === "Critical"
                ...
                {consoleMessages.map((msg, i) => (
                  <div key={i} className={msg.sender === "user" ? "text-primary" : "text-success"}>
                    <span className="text-muted-foreground font-semibold">
                      {msg.sender === "user" ? "BTP_OFFICER> " : "GRIDMIND_AI> "}
                    </span>
                    {msg.text}
                  </div>
                ))}
          ...
          <div className="relative min-h-[460px] overflow-hidden rounded-2xl border border-border panel-glass flex flex-col">
            <div className="absolute left-4 top-4 z-[500] flex items-center gap-2 rounded-lg border border-border bg-background/80 px-3 py-1.5 text-xs font-semibold backdrop-blur">
              <CircleDot className="size-3.5 text-accent animate-pulse" /> Live Tactical Map & Route
              Analysis
            </div>
            ...
            <div className="absolute bottom-3 left-3 z-[500] flex flex-wrap gap-x-3 gap-y-1 rounded-lg border border-border bg-background/80 px-3 py-2 text-[9px] backdrop-blur leading-none">
              <span className="flex items-center gap-1.5">
                <span className="size-2 rounded-full bg-success" /> Recommended detour
              </span>
              <span className="flex items-center gap-1.5">
                <span className="size-2 rounded-full bg-warning" /> Secondary
              </span>
              <span className="flex items-center gap-1.5">
                <span className="size-2 rounded-full bg-critical" /> Blocked Road
              </span>
              <span className="flex items-center gap-1.5">
                <span className="size-2 rounded-full bg-muted-foreground" /> Police station
              </span>
              {prediction && (
                <span className="flex items-center gap-1.5">
                  <span className="size-2 rounded-full bg-primary" /> AI Sourced Junctions
                </span>
              )}
            </div>
          </div>
              ...
              <div className="rounded-xl border border-border bg-input/30 p-4">
                <div className="flex items-center justify-between border-b border-border pb-2 mb-3 text-xs">
                  <span className="font-bold">
                    ACTIVE STACK ({activeStackType.toUpperCase()}) FOR{" "}
                    {selectedCorridor.name.toUpperCase()}
                  </span>
                ...
                <div className="space-y-2">
                  {selectedCorridor.stacks[activeStackType].map((route, idx) => {
                    ...
                    return (
                      <div
                        key={route.id}
                        className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-border bg-background/50 p-3 text-xs"
                      >
                        <div>
                          <div className="flex items-center gap-2">
                            <span
                              className={`text-[10px] font-bold uppercase rounded px-1.5 py-0.5 ${isPrimary
                        ...
                        <div className="flex items-center gap-3">
                          <div className="text-right">
                            ...
                            <span
                              className={`text-xs font-extrabold ${route.strikes >= 3 ? "text-critical" : route.strikes > 0 ? "text-warning" : "text-muted-foreground"}`}
                            >
                              {route.strikes}/3
                            </span>
                          </div>
                          <div>
                            ...
                            <span
                              className={`text-[10px] font-bold uppercase rounded px-1.5 py-0.5 ${route.status === "Active"
                                  ? "bg-success/15 text-success"
                                  : route.status === "Penalty Box"
                    ...
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      <div className="rounded-lg border border-success/30 bg-success/5 p-2">
                        ...
                        <p className="text-lg font-extrabold text-success">
                          ...
                          {Math.max(
                            0,
                            queryResponse.congestionBefore - queryResponse.congestionAfter,
                          )}
                          %
                        </p>
                      </div>
          ...
          <div className="w-full rounded-2xl border border-border panel-glass px-6 py-4 flex flex-wrap items-center justify-between gap-4">
            <div>
              <h3 className="text-sm font-bold text-foreground">
                AI Incident Operational Assessment
              </h3>
              <p className="text-xs text-muted-foreground mt-0.5">
                Run explainable models, green corridor allocations, and similarity matches for the
                active incident.
              </p>
            </div>
            <button
          ...
          <div ref={resultsRef} className="mt-8 space-y-6 border-t border-border pt-8">
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-border panel-glass px-5 py-4">
              ...
              >
                {scheduled ? (
                  <>
                    <Check className="size-4" /> Sourced to Planner
                  </>
                ) : (
                  <>
                    <CalendarPlus className="size-4" /> Schedule this event
                  </>
                )}
              </button>
            </div>
  ...
  return (
    <Card title="AI Congestion Impact" icon={<Activity className="size-4" />}>
      <div
        ...
        style={{ borderColor: `${sev}66`, background: `${sev}1a` }}
      >
        <div>
          <p className="text-[11px] uppercase tracking-wide text-muted-foreground">
            Severity assessment
          </p>
          <p className="text-2xl font-extrabold" style={{ color: sev }}>
            {p.severity}
          </p>
          <p className="mt-1 text-xs text-muted-foreground">
            {p.venue.name} · {p.venue.area}
          </p>
        </div>
        <div className="relative grid size-20 place-items-center">
          ...
          <span className="absolute text-mono text-lg font-bold" style={{ color: sev }}>
            {p.score}
          </span>
        </div>
      </div>
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        {stats.map((s) => (
          <div key={s.label} className="rounded-xl border border-border bg-input/30 p-3">
            ...
            <p className="text-mono text-xl font-bold">
              {s.value}
              <span className="ml-1 text-xs font-normal text-muted-foreground">{s.sub}</span>
            </p>
          </div>
        ))}
      </div>
    ...
    <Card title="Resource Recommendation" icon={<ShieldAlert className="size-4" />}>
      <div className="mb-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
        {res.map((x) => (
          <div
            key={x.label}
            className="rounded-xl border border-border bg-input/30 p-3 text-center"
          >
            <div className="mx-auto mb-1 flex size-8 items-center justify-center rounded-lg bg-primary/15 text-primary">
              {x.icon}
            </div>
      ...
      {r.deficit > 0 ? (
        <div className="mb-4 flex items-center gap-2 rounded-lg border border-critical/40 bg-critical/15 px-3 py-2.5 text-xs font-semibold text-critical">
          <AlertTriangle className="size-4" /> SHORTAGE: {r.deficit} officers — escalate to city
          reserve.
        </div>
      ) : (
      ...
      <div className="space-y-2">
        {r.allocations.map((a) => (
          <div
            key={a.station}
            className="flex items-center justify-between rounded-lg border border-border bg-input/20 px-3 py-2 text-sm"
          >
            <span className="flex items-center gap-2">
              <MapPin className="size-3.5 text-accent" /> {a.station}
            </span>
            <span className="flex items-center gap-3 text-xs text-muted-foreground">
              <span className="text-mono font-bold text-foreground">{a.officers} off.</span>
              <span className="flex items-center gap-1">
                <Clock className="size-3" /> {a.responseMin}m
              </span>
            </span>
          </div>
        ))}
  ...
  return (
    <Card title="Explainable AI" icon={<Brain className="size-4" />}>
      <p className="mb-4 text-xs text-muted-foreground">
        Why this severity? Top contributing signals, weighted.
      </p>
      <div className="space-y-3">
        {p.factors.map((f) => (
          <div key={f.label}>
...
function ChainPanel({ p }: { p: Prediction }) {
  return (
    <Card title="Chain-Reaction Predictor" icon={<Zap className="size-4" />}>
      <p className="mb-4 text-xs text-muted-foreground">
        What happens next if no intervention is made.
      </p>
      <div className="space-y-1">
        {p.chain.map((c, i) => {
          const col =
            c.risk > 0.66 ? "var(--critical)" : c.risk > 0.4 ? "var(--warning)" : "var(--success)";
          return (
            <div key={c.step}>
              <div className="flex items-center gap-3 rounded-lg border border-border bg-input/20 px-3 py-2.5">
                ...
                <span className="text-mono text-xs font-bold" style={{ color: col }}>
    ...
    <Card title="Event Similarity AI" icon={<History className="size-4" />}>
      <div className="mb-3 flex items-center justify-between rounded-xl border border-accent/40 bg-accent/10 px-4 py-3">
        <div>
          ...
          <p className="text-sm font-bold">
            {s.event.id} · {s.event.type}
          </p>
        </div>
        <span className="text-mono text-2xl font-extrabold text-accent">
          {(s.match * 100).toFixed(0)}%
        </span>
      </div>
      ...
      <div className="mt-3 rounded-lg border border-border bg-input/20 p-3">
        <p className="mb-1 flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-wide text-primary">
          <Brain className="size-3.5" /> Lesson learned
        </p>
    ...
    <Card title="Diversion & Emergency Corridor" icon={<Siren className="size-4" />}>
      ...
      <div className="space-y-2">
        {p.diversions.map((d) => (
          <div
            key={d.name}
            className="flex items-center justify-between rounded-lg border border-border bg-input/20 px-3 py-2.5"
          >
      ...
      <div className="mt-4 rounded-xl border border-info/40 bg-info/10 p-4">
        <p className="mb-2 flex items-center gap-2 text-xs font-bold uppercase tracking-wide text-info">
          <Ambulance className="size-4" /> Emergency green corridor
        </p>
        <div className="flex items-center justify-center gap-4">
          <div className="text-center">
            <p className="text-mono text-2xl font-bold text-muted-foreground line-through">
              {p.emergency.baselineMin}
            </p>
            <p className="text-[10px] uppercase text-muted-foreground">baseline</p>
          </div>
          <ArrowRight className="size-5 text-info" />
          <div className="text-center">
            <p className="text-mono text-3xl font-extrabold text-info">
              {p.emergency.optimizedMin}
            </p>
  ...
  return (
    <Card title="AI Action Plan" icon={<Megaphone className="size-4" />}>
      <div className="mb-3 flex items-center gap-2">
        ...
        <span
          className={`rounded-md border px-2 py-1 text-[10px] font-semibold uppercase tracking-wide ${aiPlan ? "border-accent/40 bg-accent/10 text-accent" : "border-border bg-input/30 text-muted-foreground"}`}
        >
      ...
      <pre className="max-h-72 overflow-auto whitespace-pre-wrap rounded-lg border border-border bg-background/60 p-3 text-mono text-[11px] leading-relaxed text-foreground/90">
        {plan}
      </pre>
      <button
        onClick={copy}
        className="mt-3 flex w-full items-center justify-center gap-2 rounded-lg border border-primary/50 bg-primary/15 px-4 py-2.5 text-sm font-semibold text-primary transition hover:bg-primary/25"
      >
        {copied ? (
          <>
          ...
          <>
            <Copy className="size-4" /> Copy action plan
          </>
        )}
      </button>
    </Card>
    ...
    <Card title="Citizen Alert System" icon={<Megaphone className="size-4" />}>
      <div className="mx-auto max-w-md rounded-2xl border border-border bg-background/70 p-4 shadow-glow">
        <div className="mb-2 flex items-center gap-2 text-warning">
          ...
          <span className="text-xs font-bold uppercase tracking-wide">Traffic Advisory</span>
        </div>
        <p className="text-sm leading-relaxed font-semibold">
          <strong>{input.type}</strong> at <strong>{p.venue.name}</strong> from{" "}
          {fmtHour(input.hour)} for {input.durationHr}h. Expect <strong>{p.delayMin} min</strong>{" "}
          delays within {p.radiusKm} km.
        </p>
        <div className="mt-3 grid grid-cols-2 gap-2 text-xs">
          ...
          <div className="rounded-lg border border-success/30 bg-success/10 px-3 py-2">
            ...
            <p className="font-semibold text-success">
              {p.diversions[0].name.replace(/^Route \w+ · /, "")}
            </p>
          </div>
        </div>
        <p className="mt-3 text-[10px] text-muted-foreground">
          — Bengaluru Traffic Police · GridMind AI
        </p>
      </div>
    </Card>
  );
  ...
  return (
    <div className="mt-10 rounded-2xl border border-dashed border-border panel-glass p-8 text-center">
      ...
      <p className="mx-auto mt-1 max-w-md text-sm text-muted-foreground">
        GridMind runs the full command-center pipeline in one click.
      </p>
      <div className="mt-5 flex flex-wrap items-center justify-center gap-2 text-xs">
        {steps.map((s, i) => (
          <span key={s} className="flex items-center gap-2">
            <span className="rounded-full border border-border bg-input/40 px-3 py-1.5 font-medium">
              {s}
            </span>

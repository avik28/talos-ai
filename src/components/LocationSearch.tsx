...
export function LocationSearch({
  value,
  onChange,
  placeholder = "Search any Bengaluru area…",
}: Props) {
        ...
        <div className="absolute z-30 mt-2 max-h-72 w-full overflow-auto rounded-xl border border-border bg-popover shadow-xl">
          ...
          {results.map((p, i) => (
            <button
              ...
              onClick={() => select(p)}
              className={`flex w-full items-center gap-3 px-3 py-2.5 text-left transition ${i === active ? "bg-primary/15" : "hover:bg-muted/60"
                }`}
            >
              <MapPin
                className={`size-4 shrink-0 ${i === active ? "text-primary" : "text-muted-foreground"}`}
              />

...
export async function fetchSupabaseIncidents() {
  ...
  return incidents.map((incident) => ({
    ...
    assigned_officers:
      incident.assigned_officers != null ? Number(incident.assigned_officers) : undefined,
    barricades_deployed:
      incident.barricades_deployed != null ? Number(incident.barricades_deployed) : undefined,
  }));
}
...
export async function fetchSupabasePoliceStations() {
  ...
  return stations.map((station) => ({
    ...station,
    available_officers:
      station.available_officers != null ? Number(station.available_officers) : undefined,
...
export async function fetchSupabaseEventPlans() {
  return supabaseFetch<SupabaseEventPlan[]>(
    "event_plans?select=*&order=created_at.desc&limit=1000",
  );
}

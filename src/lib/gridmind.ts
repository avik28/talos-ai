...
export const VENUES: Venue[] = [
  { id: "chinnaswamy", name: "M Chinnaswamy Stadium", area: "Central Zone 2", lat: 12.9788, lng: 77.5996, baseLoad: 0.92 },
  { id: "palace", name: "Palace Grounds", area: "North Zone 1", lat: 13.0048, lng: 77.5905, baseLoad: 0.8 },
  { id: "kanteerava", name: "Kanteerava Stadium", area: "Central Zone 1", lat: 12.9626, lng: 77.5946, baseLoad: 0.78 },
  { id: "freedompark", name: "Freedom Park", area: "Central Zone 1", lat: 12.9786, lng: 77.5806, baseLoad: 0.7 },
  { id: "hebbal", name: "Hebbal Grounds", area: "North Zone 2", lat: 13.0358, lng: 77.597, baseLoad: 0.74 },
  { id: "kbr", name: "KSCA / Cubbon Road", area: "Central Zone 2", lat: 12.9762, lng: 77.5929, baseLoad: 0.66 },
  { id: "whitefield", name: "Phoenix Marketcity", area: "East Zone 1", lat: 12.9959, lng: 77.6968, baseLoad: 0.6 },
];

export const STATIONS: Station[] = [
  { id: "cubbon", name: "Cubbon Park", lat: 12.9762, lng: 77.5929, officersAvailable: 15, responseMin: 6, successRate: 94, efficiency: 90 },
  { id: "halasuru", name: "Halasuru Gate", lat: 12.9784, lng: 77.6101, officersAvailable: 14, responseMin: 7, successRate: 91, efficiency: 87 },
  { id: "sadashiv", name: "Sadashivanagar", lat: 13.0064, lng: 77.5806, officersAvailable: 22, responseMin: 8, successRate: 92, efficiency: 88 },
  { id: "yeshwanth", name: "Yeshwanthpura", lat: 13.0234, lng: 77.5512, officersAvailable: 24, responseMin: 9, successRate: 88, efficiency: 84 },
  { id: "wilson", name: "Wilson Garden", lat: 12.9472, lng: 77.5986, officersAvailable: 30, responseMin: 7, successRate: 90, efficiency: 86 },
  { id: "hsr", name: "HSR Layout", lat: 12.9116, lng: 77.6412, officersAvailable: 19, responseMin: 11, successRate: 89, efficiency: 85 },
  { id: "byatara", name: "Byatarayanapura", lat: 13.0608, lng: 77.5946, officersAvailable: 23, responseMin: 10, successRate: 87, efficiency: 83 },
];

// Encode an event into a feature vector for similarity search.
function encode(type: string, attendees: number, hour: number, weekend: boolean, baseLoad: number): number[] {
...
export const PAST_EVENTS: PastEvent[] = [
  { id: "EVT-124", type: "Political Rally", venue: "Palace Grounds", attendees: 22000, hour: 16, weekend: true, delayMin: 38, officersUsed: 15, outcome: "Successful", lesson: "Outer Ring Road diversion held well; signal override at Mekhri Circle cut spillback.", vec: encode("Political Rally", 22000, 16, true, 0.8) },
  { id: "EVT-097", type: "Cricket Match", venue: "M Chinnaswamy Stadium", attendees: 35000, hour: 17, weekend: false, delayMin: 56, officersUsed: 22, outcome: "Strained", lesson: "Tow trucks arrived late at Queens Circle. Pre-position 2 tow trucks near Gate 3.", vec: encode("Cricket Match", 35000, 17, false, 0.92) },
  { id: "EVT-201", type: "Concert", venue: "Palace Grounds", attendees: 18000, hour: 19, weekend: true, delayMin: 41, officersUsed: 16, outcome: "Successful", lesson: "Staggered exit announcements reduced peak surge by ~20%.", vec: encode("Concert", 18000, 19, true, 0.8) },
  { id: "EVT-152", type: "Religious Procession", venue: "Freedom Park", attendees: 12000, hour: 9, weekend: false, delayMin: 33, officersUsed: 12, outcome: "Successful", lesson: "Rolling closures worked; keep one lane for emergency corridor.", vec: encode("Religious Procession", 12000, 9, false, 0.7) },
  { id: "EVT-178", type: "Cricket Match", venue: "M Chinnaswamy Stadium", attendees: 40000, hour: 19, weekend: true, delayMin: 62, officersUsed: 24, outcome: "Strained", lesson: "CBD saturation; widen affected radius planning to 3km and add Hosur Rd diversion.", vec: encode("Cricket Match", 40000, 19, true, 0.92) },
  { id: "EVT-066", type: "Marathon", venue: "Kanteerava Stadium", attendees: 8000, hour: 6, weekend: true, delayMin: 28, officersUsed: 14, outcome: "Successful", lesson: "Early start minimized impact; barricade volunteers helped officer load.", vec: encode("Marathon", 8000, 6, true, 0.78) },
  { id: "EVT-143", type: "VIP Movement", venue: "Palace Grounds", attendees: 3000, hour: 11, weekend: false, delayMin: 22, officersUsed: 18, outcome: "Successful", lesson: "Green corridor pre-cleared; minimal civilian disruption.", vec: encode("VIP Movement", 3000, 11, false, 0.8) },
  { id: "EVT-189", type: "Festival", venue: "Hebbal Grounds", attendees: 25000, hour: 18, weekend: true, delayMin: 49, officersUsed: 19, outcome: "Partial", lesson: "Underdeployment by ~25%; request backup earlier from Byatarayanapura.", vec: encode("Festival", 25000, 18, true, 0.74) },
];
...
function buildStationPool(stations?: HistoricalStation[]) {
  ...
  return stations
    .filter((s): s is HistoricalStation & { latitude: number; longitude: number; available_officers: number } =>
      typeof s.latitude === "number" && typeof s.longitude === "number" && typeof s.available_officers === "number",
    )
    .map((s, idx) => ({
      id: s.id || `station-${idx}`,
      name: s.name,
...
function buildHistoricalSummary(input: PredictionInput, history?: { incidents?: HistoricalIncident[]; stations?: HistoricalStation[]; plans?: HistoricalPlan[]; }): PredictionHistory {
  const incidents = history?.incidents ?? [];
  const stations = history?.stations ?? [];
  const plans = history?.plans ?? [];
  const activeIncidentCount = incidents.filter((i) => i.status && !["resolved", "closed"].includes(i.status.toLowerCase())).length;
  const relevantIncidentCount = incidents.filter((i) => {
    ...
    return active && (i.zone.toLowerCase().includes("central") || i.zone.toLowerCase().includes("north") || i.zone.toLowerCase().includes("east"));
  }).length;
  ...
  if (stations.length < 10) alerts.push("Station roster remains sparse; use conservative resource planning.");
...
export function predict(input: PredictionInput, history?: { incidents?: HistoricalIncident[]; stations?: HistoricalStation[]; plans?: HistoricalPlan[]; }, isCalibrating = false): Prediction {
  ...
  const backlogPenalty = historySummary.activeIncidentCount > 30 ? 8 : historySummary.activeIncidentCount > 15 ? 4 : 0;
  const corridorPressure = historySummary.relevantIncidentCount > 8 ? 6 : historySummary.relevantIncidentCount > 4 ? 3 : 0;

  score = Math.round(Math.min(99, score + backlogPenalty + corridorPressure));
  const severity = severityFromScore(score);
  const delayMin = Math.round(12 + (score / 100) * 70 + (isPeak ? 8 : 0) + (historySummary.relevantIncidentCount > 8 ? 4 : 0));
  const radiusKm = +(0.8 + (score / 100) * 3.2).toFixed(1);
  const recoveryHr = +(0.6 + (score / 100) * 3).toFixed(1);

  // Resources
  const officersNeeded = Math.round(6 + attendF * 28 + venue.baseLoad * 6 + (isPeak ? 3 : 0) + Math.min(6, backlogPenalty));
  ...
  const diversions: DiversionRoute[] = [
    {
      ...
      color: "var(--success)",
      points: [pair(venue, -0.02, -0.018), pair(venue, -0.01, -0.005), pair(venue, 0.006, 0.01), pair(venue, 0.022, 0.02)],
      saveMin: Math.round(delayMin * 0.55),
    },
    {
      name: "Route B · Service Bypass",
      color: "var(--info)",
      points: [pair(venue, 0.018, -0.02), pair(venue, 0.008, -0.006), pair(venue, -0.004, 0.008), pair(venue, -0.016, 0.02)],
  ...
  // Similarity search
  const vec = encode(input.type, input.attendees, input.hour, isPeak && (input.hour < 12 ? false : true), venue.baseLoad);
  const ranked = PAST_EVENTS.map((e) => ({ event: e, match: cosine(vec, e.vec) })).sort((a, b) => b.match - a.match);
  const similar = ranked[0];

  return {
    score, delayMin, severity, radiusKm, recoveryHr, factors, venue,
    resources: { officers: officersNeeded, barricades, towTrucks, ambulances, allocations, deficit },
    junctions, diversions, emergency, chain, similar,
    history: historySummary,
  };
}

function clamp(n: number) { return Math.max(0.05, Math.min(0.99, n)); }
function pt(v: Venue, dLat: number, dLng: number) { return { lat: v.lat + dLat, lng: v.lng + dLng }; }
function pair(v: Venue, dLat: number, dLng: number): [number, number] { return [v.lat + dLat, v.lng + dLng]; }
...
export function buildActionPlan(input: PredictionInput, p: Prediction): string {
  ...
  const lines = [
    ...
    ``,
    `OFFICER SOURCING`,
    ...p.resources.allocations.map((a) => `- ${a.officers} from ${a.station} (ETA ${a.responseMin} min)`),
    p.resources.deficit > 0 ? `! SHORTAGE: ${p.resources.deficit} officers — request city reserve` : `- Demand fully covered`,
    ``,
    `CRITICAL JUNCTIONS`,

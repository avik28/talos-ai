...
import type { Incident, IncidentSeverity } from "./store";
...
function locationKey(loc: string): string {
  return loc.trim().toLowerCase().split(/[\s,]+/)[0] ?? "";
}

export function computeEscalation(incident: Incident, all: Incident[], now = Date.now()): Escalation {
  ...
  const nearby = all.filter(
    (i) => i.id !== incident.id && i.status !== "Resolved" && locationKey(i.location) === key && key !== "",
  );
  ...
  const recommend = {
    ...
    towTrucks: incident.kind === "Breakdown" || incident.kind === "Accident" ? Math.max(1, Math.round(intensity * 2)) : 0,
